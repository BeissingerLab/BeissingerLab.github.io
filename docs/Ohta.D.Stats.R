...
### The index parameter is a two element vecotr in the form c(col1, col2) that specifies
...
Ohta.D.Stats <- function(index,sub){
    ...
    freqs1 <- unlist(by(sub[,1],rownames(sub),mean,na.rm=T))
...
T <- function(sub){ ### T is a function to compute Tij.s, for any specific subpopulation.
    ...
    length <- nrow(sub)
    s.00 <- length(which(sub[,1]==0 & sub[,2]==0))
